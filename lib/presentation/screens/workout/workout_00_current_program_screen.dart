...
class WorkoutCurrentProgramScreen extends ConsumerWidget {
  const WorkoutCurrentProgramScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final defaultProgramId = ref.watch(defaultProgramProvider);
    final workoutActive = ref.watch(workoutProvider.select((v) => v.active));
    final repository = ref.read(repositoryProvider);
    final program = repository.findProgram(defaultProgramId);
...
class _DaysListWidgetState extends State<DaysListWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    final defaultProgramId =
        ProviderScope.containerOf(context).read(defaultProgramProvider);
    return StreamBuilder<List<Day>>(
      stream: repository.findDaysByProgram(defaultProgramId),
      builder: (context, AsyncSnapshot<List<Day>> snapshot) {
        if (snapshot.connectionState == ConnectionState.active) {
          // Because must be mutable for sorting
          final List<Day> days = (snapshot.hasData) ? [...snapshot.data!] : [];
          if (days.isEmpty) {
            return Center(
